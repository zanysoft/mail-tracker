...
use ZanySoft\MailTracker\Events\EmailSentEvent;
...
class MailTracker implements \Swift_Events_SendListener
{
    ...
    {
        $message = $event->getMessage();
        $this->updateFailure($message, $event);
    ...
    protected function updateFailure($message, $evt)
    {
    ...
    {
        $track_all = config('mail-tracker.track-all');
        $track_by_header = strtoupper(config('mail-tracker.track-by-header'));

        if ($track_all || $track_by_header) {
            foreach ($message->getTo() as $to_email => $to_name) {
                foreach ($message->getFrom() as $from_email => $from_name) {
                    $headers = $message->getHeaders();
                    $has_tracking_header = $headers->get($track_by_header);

                    if (!$track_all && !$has_tracking_header) {
                        continue;
                    }

                    if ($has_tracking_header) {
                        $headers->remove($track_by_header);
                    }
                    ...
                    try {
                        $string_headers = $this->headersToString($headers);
                    } catch (\Exception $e) {
                        $string_headers = $headers->toString();
                    }
                    ...
                    $tracker = SentEmail::create([
                        ...
                        'newsletter_id' => $headers->get('X-NEWSLETTER-ID'),
                        'meta' => [],
                    ]);
    ...
    {
        $enable_headers = config('mail-tracker.log-headers.enable');
        $ignore_headers = config('mail-tracker.log-headers.ignore');

        if (is_bool($ignore_headers) && $ignore_headers === true) {
            return null;
        }

        $string = '';
        foreach ($headers->getAll() as $header) {
            if ($header->getFieldName() && '' != $header->getFieldBody()) {
                if ($enable_headers && is_array($ignore_headers) && in_array($header->getFieldName(), $ignore_headers)) {
                    continue;
                }
